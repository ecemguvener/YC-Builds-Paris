...
restart_pm2_app() {
  ...
  if ! command -v pm2 >/dev/null 2>&1; then
    log "❌ pm2 command not found. Install PM2 or set BARKAN_API_RESTART_CMD."
    exit 1
  fi
...
prod_root="${BARKAN_WEB_PROD_DIR:-/var/www/barkan-web}"
releases_dir="$prod_root/releases"
current_link="$prod_root/current"
api_health_url="${BARKAN_API_HEALTH_URL:-http://127.0.0.1:4000/api/health}"
api_health_retries="${BARKAN_API_HEALTH_RETRIES:-20}"
api_restart_cmd="${BARKAN_API_RESTART_CMD:-}"
...
run_quiet npm --prefix "$repo_root" --workspace @barkan/web run build
